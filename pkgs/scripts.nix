...
let
  ...
  scripts = {
    ...
    # NOTE: roll-flow / rf are no longer defined here. roll-flow is now a
    # channel-aware flake input built via ./default.nix (see channel-sources.nix)
    # and exposes the `rf`, `roll-flow`, and `roll` binaries from its own package.
  };
