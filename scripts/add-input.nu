...
def extract-packages [outputs: record, system: string = "x86_64-linux"] {
    ...
    # Check packages.*
    if ($outputs | get -i packages) != null {
        let pkg_systems = $outputs.packages
        if ($pkg_systems | get -i $system) != null {
...
def extract-overlays [outputs: record] {
    if ($outputs | get -i overlays) != null {
...
def main [
    ...
    if ($packages | is-empty) {
        ...
        let selected_packages = prompt-packages $packages $name
        ...
        for entry in ($aliases | transpose alias package) {
